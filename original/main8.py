...
def estimate_originality(text: str):
    ...
    # originality = 0.7 * (1 - max_prob) + 0.3 * ttr_score
    originality = 14 * (1 - max_prob) + 0.3 * ttr_score
    # originality = (1 - max_prob) + ttr_score + avg_tfidf_score
    # originality = 0.6 * (1 - max_prob) + 0.3 * ttr_score + 1000 * avg_tfidf_score
...
if __name__ == "__main__":
    ...
    for file in test_files:
        with open(file, 'r', encoding='utf-8') as f:
            ...
            print(f"{title} → оригинальность: {originality:.4f}, неуверенность: {p_uncertainty:.4f}, RTTR: {p_ttr:.4f}, TF-IDF: {p_tfidf:.4f}, предсказано: {predicted_author}")

    combined = list(zip(test_authors, test_titles, test_originalities, predicted_authors))
    combined.sort(key=lambda x: x[0])
    test_authors, test_titles, test_originalities, predicted_authors = zip(*combined)
